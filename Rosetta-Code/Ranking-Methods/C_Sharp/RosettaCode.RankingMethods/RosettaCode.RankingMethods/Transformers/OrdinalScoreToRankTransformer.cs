...
{
   public class OrdinalScoreToRankTransformer : IScoreToRankTransformer
   {
      ...
      {
         currentTieItemSet = currentTieItemSet
            .OrderBy( it => it.Item.Name )
            .ToList();

         foreach ( IHasScore it in currentTieItemSet )
            output.Add( new ItemWithRank( it.Item, it.Score, nextRank++ ) );
      }
